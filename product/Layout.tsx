/**
 * plugin/daraz/product/Layout.tsx
 *
 * Daraz Product Details Layout — Server Component.
 * Pre-processes product data, images, variates, seller information, and
 * category breadcrumbs, then renders the DarazProductDetails client component.
 */

import DarazProductDetails from "./Details";
...
export default function DarazProductLayout({
  data,
  settings = {},
  permalinkMap = {},
  pageData,
  ...
  const currencySymbol =
    (settings.product_currency_symbol as string) ||
    (settings.currency_symbol as string) ||
    "৳";
  ...
  const warranty = data.info?.warranty ?? "";
  ...
  const sellerPrefix =
    (permalinkMap["seller"] ?? "seller").trim().replace(/^\/+|\/+$/g, "") || "seller";

  const brandData = pageData?.brand ?? null;
  const brandPrefix = (permalinkMap["brands"] ?? "category/brands")
    .trim()
    .replace(/^\/+|\/+$/g, "");
  ...
  const brand = brandData
    ? {
        ...brandData,
        url: buildUrl(brandPrefix, brandData.slug),
      }
    : null;
