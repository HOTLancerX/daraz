...
/**
 * plugin/daraz/box/flashSaleOptional.ts
 *
 * Self-contained flash-sale types + hooks for the Daraz box components
 * (Daraz-1, Daraz-2, Daraz-3).
 *
 * Identical pattern to plugin/product/box/flashSaleOptional.ts — kept
 * separate because daraz is an independent plugin and must never import
 * from the product plugin.
 *
 * Data flow:
 *   Server: product-category serverHook fetches active campaign →
 *           pageData.flashSaleCampaign → category template → box prop
 *   Client fallback: useFlashSale() fetches /api/flash-sale?active=true
 *                    once per page load (module-level cache)
 *
 * NO imports from @/plugin/flash-sale/* or @/plugin/product/* — fully
 * self-contained.
 */
